...
class MyDatasetRole(Dataset):
    def __init__(self, args, graphs_repr_list, raw_features_list, raw_adjs_np_list, context_pairs, train_role_data_from_train_py):
        """
        args: Experiment arguments.
        graphs_repr_list: List of dicts, [{'edge_index': tensor, 'num_nodes': int}, ...].
        raw_features_list: List of NumPy arrays (node features per snapshot).
        ...
        self.device = args.device if hasattr(args, 'device') else \
                      (torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        self.features = [self._preprocess_features(feat_np) for feat_np in raw_features_list]
        self.train_role_data = train_role_data_from_train_py
        ...
        # Create a role - node id mapping for each time step
        self.time_steps = args.time_steps
        self.node_roles_map_per_timestep = [] 
        for t in range(self.time_steps): 
            current_ts_node_roles = {} # {node_id: role_id} for this timestep t
            if t in self.train_role_data: 
                for role_id, nodes_in_role_str_list in self.train_role_data[t].items():
                    for node_str in nodes_in_role_str_list: 
                        try: 
                            node_int = int(node_str) 
                            if node_int not in current_ts_node_roles:
                                current_ts_node_roles[node_int] = role_id 
                        except ValueError: 
                            print(f"Warning: Could not convert node '{node_str}' to int for role '{role_id}' at timestep {t}. Skipping this node for role mapping.") 
                            continue 
            self.node_roles_map_per_timestep.append(current_ts_node_roles) 
    ...
    def __createitems__(self):
        ...
        for node_id in self.train_nodes: # node_id is an integer from 0 to num_nodes-1
            ...
            for t_idx in range(len(node_2_all_time)): 
                ...
                node_roles_map_for_current_t = self.node_roles_map_per_timestep[t_idx]
                source_nodes_for_sampler = node_1_all_time[t_idx].cpu()
                node_negative_np = fixed_unigram_candidate_role_aware_sampler(
                    true_classes=node_positive_for_sampler.cpu(),
                    source_nodes=source_nodes_for_sampler, ### updated
                    node_roles_map=node_roles_map_for_current_t, ### updated
                    num_true=1,
                    num_sampled=self.args.neg_sample_size,
                    unique=False, 
                    distortion=0.75, 
