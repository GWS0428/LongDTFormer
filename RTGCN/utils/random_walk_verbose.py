import numpy as np
import random
import torch # For potential tensor operations if weights are tensors
...
class Graph_RandomWalk():
    def __init__(self, edge_index, num_nodes, is_directed, p, q, edge_weights=None):
        ...
        # Also store edge weights keyed by (source, target) for directed or sorted(s,t) for undirected
        ...
        for i in range(self.edge_index_np.shape[1]):
            ...
            if not self.is_directed:
                self.adj[v].append(u)
                self.edge_weights_map[(v,u)] = weight # Ensure symmetry for undirected weight lookup

        # Sort adjacency lists for consistent processing (important for alias table generation)
        for i in range(self.num_nodes):
            self.adj[i] = sorted(list(set(self.adj[i]))) # set to remove duplicates if any from edge_index

        self.alias_nodes = {}
        self.alias_edges = {}
        # Preprocessing is now called externally if needed, e.g. by run_random_walks_n2v
    ...
    def _get_edge_weight(self, u, v):
        return self.edge_weights_map.get((u,v), 0) # Return 0 if edge doesn't exist, though it should from adj

    def _has_edge(self, u, v):
        # Check based on edge_weights_map which is built from edge_index
            ...
            if cur_nbrs:
                if len(walk) == 1:
                    ...
                    if alias_key in self.alias_edges:
                        ...
                        # Fallback if (prev,cur) somehow not in alias_edges (should not happen if preprocess_transition_probs is correct)
                        # This could happen if cur has no outgoing edges after prev, or graph is disconnected
                        # print(f"Warning: Alias key {(prev,cur)} not found. Breaking walk.")
    ...
    def simulate_walks(self, num_walks, walk_length):
        ...
        for _ in range(num_walks): # walk_iter
    ...
    def get_alias_edge(self, src, dst):
        # dst is the current node in walk, src is previous. We are finding next from dst.
        unnormalized_probs = []
        # Neighbors of dst (current node)
        dst_neighbors = self._get_neighbors(dst) # Already sorted

        for dst_nbr in dst_neighbors: # For each neighbor of current node (dst)
            weight = self._get_edge_weight(dst, dst_nbr)
            if dst_nbr == src: # Moving back to previous node (src)
                unnormalized_probs.append(weight / self.p)
            elif self._has_edge(dst_nbr, src): # dst_nbr is connected to src (previous node)
                unnormalized_probs.append(weight)
            else: # dst_nbr is not connected to src
        ...
        if norm_const > 0:
            ...
            # Fallback: uniform probability if neighbors exist
            if dst_neighbors:
                normalized_probs = [1.0 / len(dst_neighbors)] * len(dst_neighbors)
            else: # Should not happen if called, implies dst_neighbors is empty
                normalized_probs = []
        ...
        if not normalized_probs and dst_neighbors: # Should not happen if norm_const > 0 path is taken correctly
            ...
            return alias_setup([1.0/len(dst_neighbors)]*len(dst_neighbors))
             ...
             # This means dst has no neighbors. alias_setup will handle empty probs list if it occurs.
             # However, the walk should break before calling this if dst_neighbors is empty.
             return alias_setup([])
    ...
    def preprocess_transition_probs(self):
        ...
        for node in range(self.num_nodes):
            ...
            if node_neighbors:
                ...
                if norm_const > 0:
                    normalized_probs = [float(u_prob) / norm_const for u_prob in unnormalized_probs]
                else: # All edge weights are zero or no neighbors with positive weight
        ...
        # Iterate through all directed edges (u,v) present in the graph
        for u_node in range(self.num_nodes):
            v_node_neighbors = self._get_neighbors(u_node)
            for v_node in v_node_neighbors: # (u_node, v_node) is an edge
                # Here, (u_node, v_node) corresponds to (prev, cur) in node2vec_walk
                # So, we need alias table for transitions from v_node, given previous was u_node
                self.alias_edges[(u_node, v_node)] = self.get_alias_edge(u_node, v_node)
                if not self.is_directed:
                    # For undirected, (v_node, u_node) is also an edge.
                    # We need alias table for transitions from u_node, given previous was v_node
