...
import torch_geometric.utils as tg_utils # For degree, add_self_loops, etc.
import numpy as np
# import scipy.sparse as sp # No longer needed for GCN normalization
...
class MyDataset(Dataset):
    ...
    def __createitems__(self):
        ...
        for node_id in self.train_nodes: # node_id is an integer from 0 to num_nodes-1
            ...
            for t in range(self.min_t, self.time_steps):
                ...
                # This can happen if node_id didn't participate in walks or has no context
                if node_id not in self.context_pairs[t] or not self.context_pairs[t][node_id]:
                    # If no context pairs, append empty lists for this timestep
                    # This ensures all lists in feed_dict have same length (number of timesteps)
                    # The loss function will handle empty tensors for a timestep if necessary.
            ...
            node_2_neg_all_time = []
            for t_idx in range(len(node_2_all_time)): # t_idx corresponds to original t from min_t to time_steps-1
                # node_2_list[t] is node_2_all_time[t_idx]
                # degree for this t_idx corresponds to self.degs[t_idx]
                
                # If node_2_all_time[t_idx] is empty, no positive samples, so no negatives needed for them.
                # However, the sampler expects true_classes. If empty, it might error or return empty.
                # The original code structure implies node_2_negative will have same length as node_2_list.
                
                if node_2_all_time[t_idx].numel() == 0: # No positive samples for this (node, t)
                ...
                node_positive_for_sampler = node_2_all_time[t_idx].unsqueeze(1) # Add dim for num_true=1

                # Ensure degs[t_idx] is available and correct
                # self.degs[t_idx] is a list of degrees for snapshot corresponding to t_idx
                # The sampler fixed_unigram_candidate_sampler expects unigrams as a list/array
                ...
                node_negative_np = fixed_unigram_candidate_sampler(
                    ...
                    unique=False, # Original DySAT used unique=False
                    distortion=0.75, # Standard value
                    unigrams=degree_dist_for_t
                ) # Returns list of np.arrays, one per positive sample
                
                # Concatenate if multiple positive samples, or handle if sampler returns flat array for batch
                # The sampler returns a list of arrays; each array is for one true_class row.
                # Here, true_classes has shape (len(node_positive_for_sampler), 1).
                # So, node_negative_np will be a list of neg_sample_size arrays.
                # We need to make it (len(node_positive_for_sampler), neg_sample_size)
                if node_negative_np and isinstance(node_negative_np, list) and \
                   all(isinstance(arr, np.ndarray) for arr in node_negative_np):
                    if all(arr.ndim > 0 for arr in node_negative_np if arr.size >0): # ensure not list of empty arrays
                    ...
                    else: # handle list of empty arrays or scalar arrays
                        # This case means fixed_unigram_sampler returned empty lists for some/all positive examples
                        # We need a tensor of shape (num_pos, num_neg_samples)
                        # If sampler returns empty arrays, create tensor of zeros or handle appropriately.
                        # For now, if it's not stackable, assume it means no valid negatives.
                        ...
                        # A better placeholder might be a special index or handling in loss.
                        # print(f"Warning: Could not stack negative samples for node {node_id}, t_idx {t_idx}. Using zeros.")

                else: # Sampler returned unexpected format or empty
                    num_pos_samples = node_positive_for_sampler.shape[0]
                    node_negative_tensor = torch.LongTensor(num_pos_samples, self.args.neg_sample_size).fill_(0).to(self.device) # Placeholder
    ...
    # num_training_batches and shuffle/test_reset are more relevant for epoch-based iteration
    # with a custom batching logic rather than PyTorch DataLoader.
    # DataLoader handles shuffling and batching.
    # If these are needed for a custom iteration scheme, they can be added back.
    # For PyTorch DataLoader, these are not typically part of the Dataset class.

    @staticmethod
    def collate_fn(samples):
        # Samples is a list of feed_dict_for_node from __getitem__
        batch_dict = {}
        if not samples: return batch_dict # Handle empty batch

        # Keys are 'node_1', 'node_2', 'node_2_neg'
        # Each is a list of tensors (one tensor per timestep)
        for key in ["node_1", "node_2", "node_2_neg"]:
            # data_list_for_key will be: [ [T0_s0, T1_s0, ...], [T0_s1, T1_s1, ...], ... ]
            # where Tx_sy is tensor for timestep x from sample y in the batch.
            data_list_for_key = [sample[key] for sample in samples if key in sample]
            
            if not data_list_for_key : continue # if key missing in all samples or samples is empty.

            concatenated_timesteps = []
            # Determine number of timesteps from the first sample, assuming consistency
            ...
            for t in range(num_timesteps_in_batch):
                ...
                # Filter out empty tensors before concatenation, if any sample had no data for this (node,t)
                # This is important if __createitems__ appends empty tensors.
                # If all are empty, cat will fail.
                ...
                if non_empty_tensors_for_t:
                    ...
                    # If all tensors for this timestep 't' across samples are empty,
                    # append an empty tensor of appropriate shape (0, ... for node_2_neg)
                    # This maintains the list length for timesteps.
                    # The shape of empty tensor depends on the key.
                    # For node_1, node_2: (0)
                    # For node_2_neg: (0, neg_sample_size) - this needs neg_sample_size
                    # This part is tricky if neg_sample_size isn't available here easily.
                    # For now, let's assume device from first tensor if possible.
                    device = samples[0][key][0].device if samples and samples[0][key] and samples[0][key][0].numel() > 0 else \
                             (torch.device("cuda" if torch.cuda.is_available() else "cpu"))

                    if key == "node_2_neg":
                        # Need neg_sample_size. If not accessible, this is problematic.
                        # Assuming it might be available via samples[0]['node_2_neg'][t].shape[1] if any non-empty.
                        # Fallback: create a (0,0) tensor if shape cannot be determined.
                        # This part of collate_fn might need adjustment based on how loss handles varying neg_sample_size or empty tensors.
                        # For simplicity, let's assume a (0) shape which might cause issues downstream if (0,N) is expected.
                        # A robust way is to ensure all items in feed_dict have consistent trailing dimensions if non-empty.
                        # Or, the model's loss must handle potentially missing negative samples for a (node,t)
