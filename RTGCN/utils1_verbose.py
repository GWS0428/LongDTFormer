...
def cross_role_hypergraphn_nodes(n_nodes, H_current_roles, role_dict1, role_dict2, Cross_role_Set, w_decay_factor, delta_t, X=None, device='cpu'):
    ...
    # H_current_roles is the incidence matrix for roles at current time t (dim n_nodes x num_roles_in_Role_set)
    # H1_cross_temporal_sparse is incidence for cross-temporal connections (dim n_nodes x num_roles_in_Cross_role_set)
    # The original code does H + H1. This implies H_current_roles and H1_cross_temporal_sparse should have compatible dimensions
    # or represent different aspects that are combined.
    # If H_current_roles is from gen_attribute_hg (nodes x Role_set_size) and H1 from cross_role (nodes x Cross_role_set_size)
    # and Role_set == Cross_role_Set, then they can be added.
    # The parameter 'H' in original 'cross_role_hypergraphn_nodes' seems to be H from current timestep's 'gen_attribute_hg'
    # If Role_set and Cross_role_Set are different, direct addition is problematic.
    # Assuming Role_set and Cross_role_Set are identical for combination:
    if H_current_roles.shape[1] == H1_cross_temporal_sparse.shape[1]:
         H_combined = H_current_roles + H1_cross_temporal_sparse
    else:
        # This case needs clarification based on how these matrices are intended to be used/combined.
        # For now, let's assume the intention was to use H1 for the "previous_role_hypergraph"
        # and H_combined might have been a conceptual sum, or H_current_roles itself IS the 'H'
        # that H1 is added to.
        # The original code returns HyperG(H1,...) and H_cross_role = H + H1.
        # This implies H (passed as H_current_roles here) and H1 have same number of columns (hyperedges).
        # This means Cross_role_Set should be used for the second dimension of H1.
        # And H_current_roles should also have n_edges = len(Cross_role_Set).
        # This indicates `H` passed to this function should already be based on `Cross_role_Set`.
        ...
        H_combined = H_current_roles # Fallback or placeholder
        # A better approach might be that H_current_roles is the H_t from gen_attribute_hg(..., Role_set,...)
        # and H1_cross_temporal_sparse is H_t-1->t from cross_role_hypergraphn_nodes(..., Cross_role_Set,...)
        # These are used separately in the RTGCN model if their hyperedge sets are different.
        # The return `previous_role_hypergraph` (HyperG(H1)) and `Cross_role_hypergraph` (H_combined) suggests
        # H1 is about the past->current links, and H_combined is the current structure enhanced by these links.
        # If len(Role_set) != len(Cross_role_Set), this addition is ill-defined.
        # The train.py uses Role_set for gen_attribute_hg and then Role_set for cross_role_hypergraphn_nodes too.
        # This implies Role_set == Cross_role_Set in that context for the H+H1 addition to make sense.
        # So, assuming H_current_roles and H1_cross_temporal_sparse have same number of columns.
...
def adj_to_edge(adjmatrix):
    ...
    if isinstance(adjmatrix, torch.Tensor):
        ...
        adjmatrix_np = np.array(adjmatrix) # Ensure it's a numpy array

    # Convert to scipy.sparse.coo_matrix to easily get row, col, data
    # This is a step we might want to avoid if performance is critical and adjmatrix is huge
    # but for moderate sizes, it's convenient.
    # Alternatively, use np.where(adjmatrix_np > 0) for unweighted, or np.nonzero for weighted
    
    # Assuming unweighted graph from original usage (v=torch.LongTensor(values))
    # If adjmatrix contains weights, values should be those weights.
    # For now, let's assume it's 0/1 and we just need indices.
    
    rows, cols = np.where(adjmatrix_np > 0) # Get indices of non-zero elements
    ...
    # If original adjmatrix had weights, use adjmatrix_np[rows, cols]
    v = torch.ones(i.shape[1], dtype=torch.long) # Original code used LongTensor for values
...
def hypergraph_role_set(train_role_graph, time_step):
    ...
    # Cross_role_Set in the original train.py was also based on Role_set
    # For clarity, if it's meant to be the same as Unique_Role_set for combination purposes:
            ...
            if len(np.unique(test_y)) > 1 : # ROC AUC requires more than 1 class in y_true
                ...
                try:
                    ...
                    if not np.array_equal(np.sort(present_classes_train), np.sort(present_classes_test)) and len(present_classes_train) > len(present_classes_test):
                        ...
                        # This is a complex case for roc_auc_score with 'ovr'.
                        # Using labels parameter for roc_auc_score can help.
                         num_classes = labels_one_hot.shape[1] # Total number of classes
